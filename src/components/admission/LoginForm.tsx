...
export default function LoginForm({ onRegisterClick, onSuccess }: LoginFormProps) {
  ...
  const handleDummyLogin = async () => {
    ...
    if (result.success) {
      ...
      setError('Dummy login failed. Please register first.');
    }
    setIsSubmitting(false);
  };
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-ui-border overflow-hidden">
        <div className="p-8 md:p-10">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-black text-text-main uppercase tracking-tight mb-2">Welcome Back</h2>
            <p className="text-sm text-text-muted font-medium">Log in to continue your application</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="text-right">
              <button type="button" className="text-[10px] font-black text-brand-primary uppercase tracking-widest hover:underline">
                Forgot Password?
              </button>
            </div>
